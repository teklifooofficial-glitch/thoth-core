...
The Thoth Core daemon can be configured to act as a trusted "border
router", implementing the thoth wire protocol and relay, making
...
The ZeroMQ feature in Thoth Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building thothd:
...
For instance:
    ...
    $ thothd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/thothd.tx.raw \
...
These options can also be provided in thoth.conf.
...
From the perspective of thothd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into thothd directly. Furthermore, no information is
...
using. Thothd appends an up-counting sequence number to each
