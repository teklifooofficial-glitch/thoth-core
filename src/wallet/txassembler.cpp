    ...
    const bool sign)
{
    ...
    while (true) {
        ...
        // the recipients are funded through the pegout kernel instead of traditional TTH outputs.
        ...
        // Calculate transaction size and the total necessary fee amount (includes TTH and MWEB fees)
    ...
    if (new_tx.mweb_type != MWEB::TxType::TTH_TO_TTH) {
...
bool TxAssembler::AttemptCoinSelection(InProcessTx& new_tx, const CAmount& nTargetValue) const
{
    ...
    if (new_tx.recipients.front().IsMWEB()) {
        ...
        if (SelectCoins(new_tx, nTargetValue, params_pegin)) {
            return std::any_of(new_tx.selected_coins.cbegin(), new_tx.selected_coins.cend(), is_tth);
        }
    } else {
        // First try to construct a TTH-to-TTH transaction
        CoinSelectionParams mweb_to_mweb = new_tx.coin_selection_params;
        mweb_to_mweb.input_preference = InputPreference::TTH_ONLY;
        ...
        // If TTH-to-TTH fails, create a peg-out transaction
